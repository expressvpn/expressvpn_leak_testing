...
class TestWindowsDNSForcePublicDNSServers(DNSDuringDisruptionTestCase):

    '''Summary:

    Test whether DNS leaks when we forcibly set the DNS servers of the primary adapter to
    use a public DNS server after connect.

    Details:

    The test connects to the VPN then sets the DNS server on the primary adapter to a public
    IP (e.g. 8.8.8.8) Once the adapter is active the test repeatedly makes DNS requests and checks
    ...
    * Write a test which sets the global DNS server but doesn't touch any adapters' DNS
      servers.
    '''
