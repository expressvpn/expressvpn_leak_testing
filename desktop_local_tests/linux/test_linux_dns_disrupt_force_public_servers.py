...
from desktop_local_tests.linux.linux_dns_force_public_resolv_conf import LinuxDNSForcePublicResolvConfDisrupter

class TestLinuxDNSForcePublic(DNSDuringDisruptionTestCase):

    '''Summary:

    Test whether DNS leaks when we forcibly set the DNS servers in resolve conf to be a public DNS
    ...
    * Write a test which sets the global DNS server but doesn't touch any network services' DNS
    ...
    def __init__(self, devices, config):
        super().__init__(LinuxDNSForcePublicResolvConfDisrupter, devices, config)
